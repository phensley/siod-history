...
;;
;; Id: siod.scm,v 1.1 1996/10/17 18:40:18 gjc Exp 
;;
;; This is a bit of maclisp compatibility. 
;; Also a somewhat broken Backquote.
;; Also anything else that seems of general use [brianbec].

;;
;; BRIANBEC: atom?, list?, list->array, array->list
;;
...
(define (list->array lyst)
  (let* ((len  (length lyst))
         (v    (vector len 'lisp)))
    (letrec ((iter (lambda (lys2 i)
                     (cond
                      ((null? lys2) v)
                      (#t (aset v i (car lys2))
                          (iter (cdr lys2) (+ i 1)))))))
...
(define (array->list arr)
  (let* ((len (length arr)))
    (letrec ((iter (lambda (built i)
                     (cond
                      ((< i 0) built)
                      (#t (iter (cons (aref arr i) built) (- i 1)))))))
            (iter '() (- len 1)))))
                          
(define vector->list array->list)

;;
;; BRIANBEC: a simple thunk iterator
;;
(define (iter-thunk thunk count)
  (define (iter-thunk-x i)
    (cond
     ((>= i count) t)
     (t (thunk) (iter-thunk-x (+ 1 i)))))
  (iter-thunk-x 0))

;;
;; BRIANBEC: a simple indexed function iterator
;;
(define (iter-monofunc monofunc count)
  (define (iter-monofunc i)
    (cond
     ( (>= i count) t )
     ( t (monofunc i) (iter-monofunc (+ 1 i)) )))
  (iter-monofunc 0))
